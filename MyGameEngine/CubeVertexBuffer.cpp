...
CubeVertexBuffer::CubeVertexBuffer() :
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(vec3d) * vertex_data.size(), vertex_data.data(), GL_STATIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(vec3d) * color_data.size(), color_data.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
